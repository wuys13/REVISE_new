...
def _annotate_OTREVISE(
    ...
    cell_prior = helper.prep_cell_priors(adata, reads=True) # count per cell
    type_cell_dist, adata_pv, reference_pv = helper.prep_distance_REVISE(adata, reference, annotation_key, decomposition=decomposition,
                                          deconvolution=deconvolution, **kw_args)
    if decomposition:
    ...
    if decomposition:  # include the annotation profiles themselves to obtain a measurement of confusion
        test_weight = 1e-6 * cell_prior.sum() / (len(type_cell_dist.columns) - len(
            cell_prior.index))  # keep a low weight to not influence the actual typing (much)
    ...
    return cell_type, adata_pv, reference_pv


def annotate_OTREVISE(
        adata,
        reference,
        annotation_key=None,
    ...
    **kw_args
    ...
    adata, reference, annotation_key = helper.validate_annotation_args(adata, reference, annotation_key,
                                                                       counts_location)

    # call typing without data integrity checks
    cell_type = _annotate_OTREVISE(
        adata=adata,
        reference=reference,
