...
def prep_cell_priors_SpaGFT_SVG(tdata, Given_SVG, reads=False):
    if reads:
        cell_prior = pd.Series(np.array(tdata[:Given_SVG].X.sum(axis=1)).flatten(), index=tdata.obs.index)
    else:
        cell_prior = None
    return cell_prior


def prep_cell_priors_SpaGFT_FC(tdata, Given_FC, reads=False):
    if reads:
        cell_prior = pd.Series(np.array(tdata.X.sum(axis=1)).flatten(), index=tdata.obs.index)
    else:
        cell_prior = None
...
def prep_distance(tdata, reference, type_key, n_pca=None, zero_center=False, log_norm=False, scale=False, metric='h2', min_distance=False, decomposition=False, deconvolution=False,S_spagft = None,lamada_mtb = None):
    ...
    if min_distance:
        ...
        average_profiles = utils.get_average_profiles(type_key, reference, rX, pca_offset, pca_trafo)  # gene * cell_type
...
def prep_distance_SpaGFT(tdata, reference, type_key, n_pca=None, zero_center=False, log_norm=False, scale=False, metric='h2', min_distance=False, decomposition=False, deconvolution=False, S_spagft = None, lamada_mtb = None):
    if type_key in reference.varm and min_distance:
        raise ValueError('Supplying the reference as average profiles does not work with min_distance!')
    if decomposition and min_distance:
        raise ValueError('The decomposition workflow does not work with min_distance!')
...
def prep_distance_REVISE(
        tdata, reference, type_key, n_pca=None, zero_center=False, log_norm=False, scale=False, metric='h2',
        min_distance=False, decomposition=False, deconvolution=False
):
    if type_key in reference.varm and min_distance:
        raise ValueError('Supplying the reference as average profiles does not work with min_distance!')
    if decomposition and min_distance:
    ...
    if do_pv:
        from ..utils._utils import get_common_embedding
        overlap_genes = list(tdata.var_names.intersection(reference.var_names))
        tdata, reference = get_common_embedding(
            tdata[:, overlap_genes], reference,
            method="svd", emb="pca",
            n_components=100, project_on="sp",
            cos_threshold=0.3,
            normalize_flag=False,
        )
        print(f"After PV, tdata: {tdata.shape}, reference: {reference.shape}")
        # rX = reference.obsm['X_shared']
        # tX = tdata.obsm['X_shared']
        rX = reference.X.copy()
        tX = tdata.X.copy()
        # type_key = "PV"
    else:
    ...
    if min_distance:
        ...
        cref_cell_dist[cref_cell_dist < 1e-10] = np.inf  # make equal cell comparisons irrelevant for the min operation
        types = reference.obs[type_key].unique()
        type_cell_dist = pd.DataFrame({l: np.min(cref_cell_dist[reference.obs[type_key] == l], axis=0) for l in types},
                                      index=tdata.obs_names.copy())[types].T
    else:
        average_profiles = utils.get_average_profiles(type_key, reference, rX, None, None)
        ...
        if deconvolution or metric in ['weighted', 'nwsp', ]:
            ...
            if do_pv:
                tX = tX @ tdata.varm['PV']
                refX = rX @ reference.varm['PV']
                _dist = utils.cdist(refX, tX, metric=metric)
            else:
                _dist = utils.cdist(average_profiles.T, tX, metric=metric)
        type_cell_dist = pd.DataFrame(_dist, index=average_profiles.columns.copy(),
                                      columns=tdata.obs_names.copy())  # have to use copies of the indices as otherwise there is still a reference to temporary objects from within the function, which cannot be cleaned up by the garbage collector...

        if decomposition:
            mixture_profiles, mixtures = utils.generate_mixture_profiles(average_profiles.to_numpy(),
                                                                         include_pure_profiles=(decomposition < 2))
            type_mix_dist = pd.DataFrame(utils.cdist(average_profiles.T, mixture_profiles, metric=metric),
                                         index=average_profiles.columns.copy())
            type_cell_dist = pd.concat([type_cell_dist, type_mix_dist], axis=1)
    ...
    if decomposition:
        ...
        return type_cell_dist, tdata, reference
