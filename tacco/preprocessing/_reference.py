...
def construct_reference_profiles(
    ...
    # if annotate reference single cell, this will not be passed
    if reference.obs[annotation_key].nunique() != reference.obs.shape[0]:
        ...
        profiles = pd.DataFrame(reference.X.T.copy().todense(), index=reference.var.index, columns=reference.obs[annotation_key].values.tolist())
