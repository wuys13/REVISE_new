...
def _run_OT(type_cell_dist, type_prior=None, cell_prior=None, epsilon=5e-3, lamb=None, inplace=False):
    ...
    cost /= cost.max()
    # np.save('/bmbl_data/jiangyi/SpaGFT/TACCO_Update/test_data/cost.npy',cost)
...
def get_average_profiles(
    ...
    if type_key in reference.varm:
        ...
        if len(found_key) == 1: # only use this hit, if there was no better hit before
            if type(reference.varm[type_key]) == pd.DataFrame:
                mean_profiles = reference.varm[type_key].fillna(0)
            else:
                mean_profiles = reference.varm[type_key]
...
def get_common_embedding(
    ...
    if zscore_flag:
        sp_data = scipy.stats.zscore(sp_data, axis=0)
        sc_data = scipy.stats.zscore(sc_data, axis=0)
    ...
    if method == "svd":
        ...
        print(
            f"Max: {np.max(cos_similarity):.4f}, Min: {np.min(cos_similarity):.4f}, Median: {np.median(cos_similarity):.4f}")
        # print(cos_similarity)
        effective_n_pv = np.sum(cos_similarity > cos_threshold)
        print(
            f"PV: Selected {effective_n_pv} effective components based on cosine similarity threshold {cos_threshold}")
    ...
    if project_on == "sc":
        ...
        adata_sp_raw.varm["PV"] = pd.DataFrame(sc_pv.T, index=adata_sp_raw.var.index.copy())
        adata_sc_raw.varm["PV"] = pd.DataFrame(sc_pv.T, index=adata_sc_raw.var.index.copy())
    elif project_on == "sp":
        adata_sp_raw.obsm["X_shared"] = sp_data @ sp_pv.T
        adata_sc_raw.obsm["X_shared"] = sc_data @ sp_pv.T

        adata_sp_raw.varm["PV"] = pd.DataFrame(sp_pv.T, index=adata_sp_raw.var.index.copy())
        adata_sc_raw.varm["PV"] = pd.DataFrame(sp_pv.T, index=adata_sc_raw.var.index.copy())
